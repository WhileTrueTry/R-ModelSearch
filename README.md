...
Este es un módulo de R que automatiza la búsqueda de modelos lineales (lm) para una variable objetivo, probando múltiples transformaciones y complejidades mediante una estrategia de fuerza bruta. Es poco eficiente, pero útil en casos puntuales donde se consideran pocas variables. 

Su objetivo es facilitar un análisis exploratorio, y es importante recalcar que a menudo un análisis detallado puede encontrar que existen modelos más sencillos, con valores de R² muy cercanos, y que al ser menos complejos pueden ofrecer mayor interpretabilidad. Además, la idea es probar combinaciones sencillas y anidadas, pero otro tipo de combinaciones más complejas (por ejemplo, mezclando interacción entre algunas variables pero no entre otras) pueden ser más útiles.
